...
"""Diffusion Probabilistic Model Implementation

This module implements the diffusion model described in Section 2 (Nonequilibrium Thermodynamics) 
of the paper 'Deep Unsupervised Learning using Nonequilibrium Thermodynamics'.

The model defines a flexible and tractable generative model by gradually converting a simple 
distribution into a complex data distribution through an iterative diffusion process.

Key Components:
1. Forward process q(x_t|x_0): Section 2.1
   - Markov chain that gradually converts data distribution into Gaussian noise
   - Uses variance schedule β_t that increases linearly from β₁ to βₜ
   - Each step adds a small amount of Gaussian noise according to β_t
   
2. Reverse process p(x_{t-1}|x_t): Section 2.2
   - Learns to reverse the diffusion process to recover the original data
   - Neural network predicts mean μ_θ(x_t,t) and variance σ_θ²(x_t,t)
   - Uses Gaussian time encoding to handle different noise levels
   
3. Training objective: Section 2.3
   - Minimizes negative log-likelihood -log p(x_0)
   - Uses reparameterization trick for efficient training
   - Uniformly samples timestep t for stochastic optimization

Example Usage:
    # Create model for MNIST-like images (28x28, grayscale)
    >>> model = DiffusionModel(
    ...     spatial_width=28,        # Image size
    ...     n_colors=1,              # Grayscale
    ...     trajectory_length=1000,   # Number of diffusion steps
    ...     beta_start=0.0001,       # Initial noise level
    ...     beta_end=0.02,           # Final noise level
    ... )
    
    # Training: Forward process and loss calculation
    >>> x = torch.randn(32, 1, 28, 28)  # Batch of images
    >>> loss = model.cost_single_t(x)    # Calculate training loss
    >>> loss.backward()                  # Backpropagate
    
    # Sampling: Generate new images
    >>> samples = model.sample(batch_size=16)  # Generate 16 new images
    >>> samples.shape  # torch.Size([16, 1, 28, 28])
    
    # Get noisy version of images at specific timestep
    >>> x = torch.randn(8, 1, 28, 28)           # Original images
    >>> t = torch.tensor([500])                  # Timestep
    >>> noisy_x, noise, _, _ = model.forward_process(x, t)
    >>> noisy_x.shape  # torch.Size([8, 1, 28, 28])
    
    # Predict denoising parameters
    >>> mu, sigma = model.get_mu_sigma(noisy_x, t)
    >>> mu.shape     # torch.Size([8, 1, 28, 28])
    >>> sigma.shape  # torch.Size([8, 1, 28, 28])

References:
    [1] Sohl-Dickstein et al. "Deep Unsupervised Learning using Nonequilibrium Thermodynamics"
    [2] Algorithm 1: Sampling procedure
    [3] Equation 2: Forward process
    [4] Equation 5: Reverse process
    [5] Equation 9: Training objective
"""

class DiffusionModel(nn.Module):
    """Diffusion Probabilistic Model based on 'Deep Unsupervised Learning using Nonequilibrium Thermodynamics'
    
    Implementation of the diffusion model described in Section 2 (Nonequilibrium Thermodynamics) of the paper.
    The model defines a flexible and tractable generative model by gradually converting a simple distribution
    into a complex data distribution through an iterative diffusion process.
    
    Key Components:
    1. Forward process q(x_t|x_0): Section 2.1
       - Markov chain that gradually converts data distribution into a simple Gaussian noise
       - Uses a variance schedule β_t that increases linearly from β₁ to βₜ
       - Each step adds a small amount of Gaussian noise according to β_t
